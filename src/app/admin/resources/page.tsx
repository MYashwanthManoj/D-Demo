...
export const metadata = {
  title: "Resources Management — Admin | Career Cafe"
}
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-navy-500 mt-1">Manage articles and guides.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-navy-100">
              {resources.map(resource => (
                <tr key={resource.id} className="hover:bg-navy-50/50 transition-colors">
                  <td className="p-4">
                    <span className="font-medium text-navy-900 block line-clamp-1 max-w-sm">{resource.title}</span>
                  </td>
                  <td className="p-4">
                    <span className="text-sm text-navy-600 block">{resource.category}</span>
                    <span className="text-xs text-navy-400">{resource.type}</span>
                  </td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-md text-xs font-medium ${
                      resource.status === 'PUBLISHED' ? 'bg-emerald-50 text-emerald-700' :
              ...
              {resources.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-navy-500">No resources found.</td>
                </tr>
              )}
            </tbody>
          </table>
